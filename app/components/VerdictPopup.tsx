...
export default function VerdictPopup({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-slate-900/30 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-primary-bg w-full max-w-xl rounded-4xl p-6 flex flex-col items-center gap-8 shadow-2xl border-4 border-white/80 animate-in zoom-in-95 duration-300">
        <div>
          <p className="text-xs font-black text-tertiary uppercase tracking-widest">
            Your Excuse Scored
          </p>

          {/* Score Header */}
          <div className="flex items-center gap-3 mb-6">
            <div className="text-6xl font-black text-primary flex items-baseline">
              <span>{verdict.score}</span>
              <span className="text-4xl text-tertiary mx-1">/</span>
              <span className="text-4xl text-tertiary">100</span>
            </div>
        ...
        <div className="w-full space-y-4 mb-6">
          {criteria.map((item, index) => (
            <div key={index} className="w-full flex flex-col gap-1.5">
              <div className="flex justify-between items-center text-[11px] font-black text-tertiary uppercase">
                <span>{item.name}</span>
                <span>
                  {item.score}/{item.max}
                </span>
        ...
        <div className="w-full flex flex-col gap-8">
          ...
          <div className="flex gap-3 w-full">
            <Link
              href={"/"}
              className="flex-1 flex items-center justify-center gap-2 bg-slate-100 text-tertiary py-3 rounded-xl font-bold hover:bg-slate-200 hover:text-secondary active:scale-95 transition-all"
            >
