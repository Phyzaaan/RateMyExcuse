  ...
  return (
    <main className="flex flex-col max-w-5xl w-full min-h-screen text-primary gap-6 p-4">
      <div className="flex items-center justify-between gap-3">
        <Link
          href="/"
          className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/70 px-3 py-2 text-sm font-bold text-primary shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
        >
      ...
      <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3 pb-8">
        {loading ? (
          ...
          posts.map((post) => (
            <CommunityPostCard key={post.id} item={post} setToast={setToast} className="w-full" />
          ))
        ) : (
